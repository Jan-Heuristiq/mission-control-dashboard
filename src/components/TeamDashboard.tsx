...
type EditPostFormProps = {
    post: Post;
    onSave: (text: string, type: 'win' | 'blocker') => void;
    onCancel: () => void;
    isSubmitting: boolean;
};
...
const TeamDashboard: FC<TeamDashboardProps> = ({ data, addPost, updatePost, deletePost, isSubmitting, loggedInUser }) => {
    ...
    const totalProgress = totalTarget > 0 ? (totalRevenue / totalTarget) * 100 : 0;
    const thisMonthTarget = totalTarget / totalMonths;
    ...
    }, [revenueEntries, currentMonth, sprintStartYear, sprintStartMonth]);

    const thisMonthProgress = thisMonthTarget > 0 ? (thisMonthRevenue / thisMonthTarget) * 100 : 0;
    const contributionData = useMemo(() => founders.map(founder => ({ name: founder.name, value: revenueEntries.filter(entry => entry.founder_id === founder.id).reduce((sum, entry) => sum + entry.amount, 0) })).filter(d => d.value > 0), [founders, revenueEntries]);
    ...
    const radialData = [{ name: 'Month', value: Math.min(thisMonthProgress, 100) }];
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-[#F5F4EF] p-6 rounded-xl border border-[#929A8A]/50 shadow-lg">
                <h2 className="text-lg sm:text-xl font-bold text-[#1A1A1A] mb-2">{`Our Mission: ${formatCurrency(totalRevenue)} / ${formatCurrency(totalTarget)}`}</h2>
                <div className="w-full bg-[#929A8A]/30 rounded-full h-6 relative overflow-hidden">
                    <div className="bg-[#C4FF00] h-6 rounded-full text-center text-[#1A1A1A] text-sm flex items-center justify-center font-bold transition-all duration-1000 ease-out" style={{ width: `${Math.min(totalProgress, 100)}%` }}>
                        <span className="z-10">{`${totalProgress.toFixed(1)}%`}</span>
                    </div>
                </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1 bg-[#F5F4EF] p-6 rounded-xl border border-[#929A8A]/50 shadow-lg flex flex-col">
                    <h3 className="text-lg font-semibold mb-4 text-center">This Month's Momentum</h3>
                    <ResponsiveContainer width="100%" height={250}>
                        <RadialBarChart innerRadius="70%" outerRadius="100%" data={radialData} startAngle={180} endAngle={0} barSize={30}>
                            ...
                            <text x="50%" y="70%" textAnchor="middle" dominantBaseline="middle" className="fill-[#1A1A1A] text-4xl font-bold">{`${thisMonthProgress.toFixed(0)}%`}</text>
                ...
                <div className="lg:col-span-2 bg-[#F5F4EF] p-6 rounded-xl border border-[#929A8A]/50 shadow-lg flex flex-col">
                    <h3 className="text-lg font-semibold mb-4 text-center flex items-center justify-center gap-2"><ChartPieIcon className="h-6 w-6 text-[#004225]" /> Revenue Sources</h3>
                    <ResponsiveContainer width="100%" height={250}>
                        <PieChart>
                            <Pie data={contributionData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} fill="#8884d8" paddingAngle={5} dataKey="value" nameKey="name">
                                {contributionData.map((_entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} className="focus:outline-none" />)}
                            </Pie>
